...
This section contains the links to the sources of the data we are employing, followed by the code to load that data and some first steps of manipulation for the sake of efficiency. The datasets derived from this stage are all stored in the `data` folder of the github project. 
...
| Percentage of missing values over the total | `r sum(is.na(zhvi_data)) / sum(!is.na(zhvi_data))` |
...
It provides risk metrics for 18 hazard types at the census-tract geographical level, meaning it provides multiple observations for each county. Included in the dataset are also the county, state, and census tract FIPS code.
...
| Percentage of missing values over the total | `r sum(is.na(NRI_dataset)) / sum(!is.na(NRI_dataset))` |
...
| Percentage of missing values over the total | `r sum(is.na(SAIPE)) / sum(!is.na(SAIPE))` |
...
| Percentage of missing values over the total | `r sum(is.na(population)) / sum(!is.na(population))` |
...
| Percentage of missing values over the total | `r sum(is.na(building_permits)) / sum(!is.na(building_permits))` |
