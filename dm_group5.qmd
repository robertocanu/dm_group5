...
```{r}
NRI_dataset <- vroom('/Users/robertocanu/Desktop/dm_group5/data/National_Risk_Index_Census_Tracts_-2688098782675405028.csv', 
...
```{r}
temporary <- read_excel('/Users/robertocanu/Desktop/dm_group5/data/est19all.xls',
  sheet = 1, skip = 3)
# head(temporary)
readr::write_csv(temporary, '/Users/robertocanu/Desktop/dm_group5/data/SAIPE.csv')  
SAIPE <- vroom('/Users/robertocanu/Desktop/dm_group5/data/SAIPE.csv')
```
...
```{r}
population <- vroom('/Users/robertocanu/Desktop/dm_group5/data/co-est2020-alldata.csv',
                    col_types = cols(id = col_character(), 
                               date = col_date(), 
                               value = col_double()))
...
```
The data contains raw numbers for the total population and the net change by county, from 2010 to 2020. We are interested in having rates of change which can be more reliably compared. For this purpose, we create an additional variable for each year, computed as the population change in year `x` divided by the population estimate in year `x-1`. 
For clarity, the U.S. Bureau of Census publishes the estimates as the levels on the 1st of July of each year, so that the 2010's population estimate is the total on 1st of July 2010 and the population change in 2010 is the change between 1st of July 2009 and 1st of July 2010.
...
```{r}
building_permits <- vroom('/Users/robertocanu/Desktop/dm_group5/data/Residential_Construction_Permits_by_County_5026727375813176131.csv', 
              col_types = cols(id = col_character(), 
                               date = col_date(), 
                               value = col_double()))
```
