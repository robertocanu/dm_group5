...
## Github link
The project can be accessed on Github at the following link: [Discount in Housing Prices in Areas Exposed to Natural Disasters across the US](https://github.com/robertocanu/dm_group5)

## Research Question 
We intend to investigate whether the exposure to natural disasters creates a discount in housing prices, such that people’s reservation price in areas more at risk is lower. To do so, we want to regress  the Zillow Home Value Index - (which is a measure of the typical value of a house - hence of houses between the 35th and the 65th percentile of values – adjusted for price differences across the country) on the national risk index (a measure of exposure to natural disasters), while including a set of controls influencing prices (as state-level effects from different laws and regulations, poverty rates, building permits, median income). 
Then, we want to deepen the research by analyzing how the discount varies depending on how rich the area is. Since poorer people have far fewer opportunities to get insurance—and even when they do, they tend to insure only a smaller share of their belongings—and because they usually own properties that are less well-prepared for natural disasters (cheaper houses built with lower-quality materials and outdated technologies), the negative impact on prices discussed earlier should be stronger in poorer counties (namely, a higher share of people having notably lower reservation costs for houses). Then, we intend to test the difference in effect between areas according to their income. We will do so either by using an interaction effect for the national risk index and the income within the main regression (adding some controls if needed) or by regressing a top and a bottom percentile in terms of income and testing differences between them. 

## Datasets Used & Main Variables
The datasets we intend to use so far are five. They all present data from the United States at the county level (3144 counties), which will be merged using the FIPS code for each county.  
The first two are the ones comprising the dependent variable (Zillow Home Value Index - ZHVI) and the independent one (Natural Risk Index - NRI). The other three are datasets containing various controls that we intend to use in the regressions.  
NRI is available only for 2020; since it is constructed with data from 2014 to 2019 and it is inherently a slowly changing index per se (exposure to natural disasters does not change year by year), we decided to employ data from 2019 for most of the remaining variables, so to avoid shocks to the housing markets due to Covid to bias our results. Specifically, we will employ the Zillow index, the median income and the poverty rates of 2019. Instead, we will create an average from 2014 to 2018 for both the population change rate and the building permits granted; indeed, we want to use these two as controls and we believe that a number taking into account more of the previous years is more informative and prevent non-common spikes to weaken our controls. 


# Data Loading & Sources 

This section contains the links to the sources of the data we are employing. It follows the code to load that data and some first steps of preliminary data preparation with specific explanations behind their motive. The datasets derived from this stage are all stored in the `data` folder of the github project. 

## [Zillow Home Value Index (ZHVI) Data](https://www.zillow.com/research/data/?msockid=11606803e4976e040d5e7e84e5176f46)

### Data Retrieval 

To retrieve the dataset, one has to follow the link and download the data from the first section **HOME VALUES** selecting the default *data type* `ZHVI All Homes (SFR, Condo/Co-op) Time Series, Smoothed, Seasonally Adjusted ($))` from the first dropdown menu and the regional option `County` under the second menu, *Geography*.
...
### Data Description
The Zillow Home Value Index (ZHVI) is a monthly, seasonally adjusted measure of the typical home value (homes within the 35th and 65th percentile range) for each U.S. county. In the dataset, counties are identified through the variables StateCodeFIPS and MunicipalCodeFIPS, which can be combined to get a standard five-digit county FIPS code that is used to identify the counties across all the datasets we use. 

The index is monthly, and we use the values of 2019, averaged, to get our dependent variable. 

For what concerns the measure per se, the ZHVI is a preferable figure of housing prices compared to median house prices because it reflects the estimated value of all homes, not just those recently sold. This makes it less sensitive to changes in the composition of transactions (for example, if only high-end homes sell in a given month). Thus, it provides a more stable and comprehensive measure of overall housing market value across counties. 

### Data Summary Table 

| Descriptive Statistic | Value | 
|--------|-------|
| Number of observations | `r nrow(zhvi_data)` |
| Number of variables | `r ncol(zhvi_data)` |
| Number of missing values | `r sum(is.na(zhvi_data))` | 
| Number of non-null values | `r sum(!is.na(zhvi_data))` | 
| Percentage of missing values  | `r sum(is.na(zhvi_data)) / sum(!is.na(zhvi_data))` |
...
The dataset can be easily and directly retrieved from the page linked. Being it significantly large (452,2 MB), we couldn't include it on the github project. We thus decided to create a temporary file on the project to reduce its size, given we were not interested in most variables in it, and then include the deriving dataset (`NRI_dataset2`) in the project. 


### Data Description

The FEMA National Risk Index (NRI) is a nationwide dataset that estimates the relative risk of natural hazards across the U.S. It combines information on hazard frequency, exposure, expected annual losses (for buildings, people, and agriculture), and community resilience and social vulnerability. It provides risk metrics for 18 hazard types at the census-tract geographical level, meaning it provides multiple observations for each county. Included in the dataset are also the county, state, and census tract FIPS code.

The core metric of interest in the dataset is the NRI itself, which is a composite score ranging from 0 to 100 that reflects each area’s overall exposure and vulnerability to natural hazards. Another metric is the `Rating` of the index, which assigns to every county a qualitative category — from “Very Low” up to “Very High” — that describes how its risk compares with all other counties in the United States. This is derived converting the value into a percentile score (0–100) that ranks the county relative to all others. Finally, the score is grouped into one of five qualitative `Rating` categories using a clustering method (such as natural breaks), producing the final label from *Very Low* to *Very High*. We employ the rating only to undertake a comparison to the simpler measure that we will use as the independent variable, namely a rating based directly on the county’s risk percentile.
...
| Descriptive Statistic | Value | 
|--------|-------|
| Number of observations | `r nrow(NRI_dataset)` |
| Number of variables | `r ncol(NRI_dataset)` |
| Number of missing values | `r sum(is.na(NRI_dataset))` | 
| Number of non-null values | `r sum(!is.na(NRI_dataset))` | 
| Percentage of missing values  | `r sum(is.na(NRI_dataset)) / sum(!is.na(NRI_dataset))` |
...
Once in the page linked, the dataset can be retrieved selecting under the section **United States Summaries** the option `US and All States and Counties`. 

We decided to compile the `.xsl` file we downloaded into a `.csv` to better leverage the function `vroom` and improve efficiency. We also dropped the first two rows of the excel file because they present information on the dataset (which have been integrated in the description above) and prevent the right compilation in categories into the `.csv` file.
...
| Descriptive Statistic | Value | 
|--------|-------|
| Number of observations | `r nrow(SAIPE)` |
| Number of variables | `r ncol(SAIPE)` |
| Number of missing values | `r sum(is.na(SAIPE))` | 
| Number of non-null values | `r sum(!is.na(SAIPE))` | 
| Percentage of missing values  | `r sum(is.na(SAIPE)) / sum(!is.na(SAIPE))` |
...
### Data Retrieval & Loading
To retrieve the dataset, one must donwload under the section **Annual Resident Population Estimates, Components of Change, and Rates of Population Change** the file `Annual Resident Population Estimates, Estimated Components of Resident Population Change, and Rates of the Components of Resident Population Change for States and Counties: April 1, 2010 to July 1, 2020 (CO-EST2020-alldata)`. 


### Data Description & Preliminary Data Preparation

The dataset contains raw numbers for the total population and the net change by county, from 2010 to 2020. Then, given we intend to use a measure of population change across the years before 2019 as a control for the increase in housing prices, we created a new variable being the rate of change for each year (called `POPCHGRATE_XXXX`). We compute this variable as the population change in year `x` divided by the population estimate in year `x-1`. For clarity, the U.S. Bureau of Census publishes the estimates as the levels on the 1st of July of each year, so that the 2010's population estimate is the total on 1st of July 2010 and the population change in 2010 is the change between 1st of July 2009 and 1st of July 2010.
We also combined the state and county codes to get the complete FIPS code. 
...
| Number of non-null values | `r sum(!is.na(population))` | 
| Percentage of missing values | `r sum(is.na(population)) / sum(!is.na(population))` |
...
### Data Retrieval & Loading
After having accessed the page linked, to retrieve the dataset one has to click on the download icon on the red sidebar, then ensure to activate the filters (which have already been selected ) before downloading. These filters restrict the years for which the data is downloaded (2014 to 2019). 
...
### Data Description 
The variable we are interested in is `ALL_PERMITS_XXXX`, which provides the raw number of construction permits granted within a county during the specified year. Two other variables are included: the geographic code (coordinates) and the county code. This dataset, as evident in the table below, present an extremely large number of missing values for the large number of variables, which we get rid of in the data cleaning process, that consider specific sections of the permits and for which data is available only in bigger counties. It will significantly diminish when only the variable containing all permits is kept. 
...
| Descriptive Statistic | Value | 
|--------|-------|
| Number of observations | `r nrow(building_permits)` |
| Number of variables | `r ncol(building_permits)` |
| Number of missing values | `r sum(is.na(building_permits))` | 
| Number of non-null values | `r sum(!is.na(building_permits))` |
| Percentage of missing values  | `r sum(is.na(building_permits)) / sum(!is.na(building_permits))` |



# Data Cleaning

This section contains the manipulation we did on the datasets to get the final merged version. Moreover, it discusses the characteristics of each dataset and present graphical representations for our main target variable, the `Zillow Value Index` of housing prices, and our independent variable, the `NRI Index`. 
...
Keeping only relevant variables to identifying counties and the observations for each month in the year 2019
...
The number of missing values in the dataset is extremely small:
```{r}
100 * mean(is.na(pop_df))
```

# Data Merging

Making sure each of the datasets has the same variable name for the FIPS identifier

```{r}
zhvi_clean <- zhvi_clean %>%
  rename(FIPS = CountyCodeFIPS)
```
